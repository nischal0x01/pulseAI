...
def plot_predictions_vs_actual(y_true, y_pred, dataset_name="Test", save_path=None):
    ...
    # Scatter plot
    ax.scatter(y_true, y_pred, alpha=0.6, edgecolors='k', s=50, label='Predictions')
    ...
    ax.set_title(f'{dataset_name} Set: Predicted vs Actual SBP\n' +
...
def plot_error_distribution(y_true, y_pred, dataset_name="Test", save_path=None):
    ...
    axes[0].set_title(f'{dataset_name} Set: Error Distribution', 
                      fontsize=14, fontweight='bold')
    axes[0].legend()
    ...
    axes[1].set_title(f'{dataset_name} Set: Error Box Plot', 
                      fontsize=14, fontweight='bold')
...
def comprehensive_evaluation(model, X_test, y_test, dataset_name="Test", 
    ...
    try:
        ...
        if hasattr(model, 'output_names') and len(model.outputs) > 1:
            y_pred, attention_weights = model.predict(X_test, verbose=0)
    ...
    plot_predictions_vs_actual(y_test, y_pred, dataset_name, save_path)
    
    # Plot error distribution
    save_path = f"{save_dir}/error_dist_{dataset_name.lower()}.png" if save_dir else None
    plot_error_distribution(y_test, y_pred, dataset_name, save_path)
